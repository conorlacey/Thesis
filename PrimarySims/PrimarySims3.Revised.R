...
for (i in 1:500){
  for (cond in 1:54){
    ...
    if (cond %in% c(1,2,3,10,11,12,19,20,21,
                    28,29,30,37,38,39,46,47,48)){ # dMACS 0.1 mean equal
      load.R <- .6 #loading for the reference group
      load.F <- .65 #loading for focal group
      int.R <- 0 #intercept for reference group
      int.F <- 0.08 #intercept for focal group
    }
    if (cond %in% c(4,5,6,13,14,15,22,23,24,
                    31,32,33,40,41,42,49,50,51)){ #dMACS 0.5 mean equal
      load.R <- .6 #loading for the reference group
      load.F <- .67 #loading for focal group
      int.R <- 0 #intercept for reference group
      int.F <- 0.49 #intercept for focal group
    }
    if (cond %in% c(7,8,9,16,17,18,25,26,27,
                    34,35,36,43,44,45,52,53,54)){ #dMACS 0.9 mean equal
      load.R <- .6 #loading for the reference group
      load.F <- .67#loading for focal group
      int.R <- 0 #intercept for reference group
      int.F <- 0.9 #intercept for focal group
    }
...
saveRDS(dat_dMACS, file = "data/dat_dMACS.Rev3.RDS")
saveRDS(dat_dMACS_low_CI, file = "data/dat_dMACS_low.Rev3.RDS")
saveRDS(dat_dMACS_high_CI, file = "data/dat_dMACS_high.Rev3.RDS")

saveRDS(dat_dMACS_post, file = "data/dat_dMACS_post.Rev3.RDS")
saveRDS(dat_dMACS_low_CRI, file = "data/dat_dMACS_low_CRI.Rev3.RDS")
saveRDS(dat_dMACS_high_CRI, file = "data/dat_dMACS_high_CRI.Rev3.RDS")

saveRDS(dat_dMACS_S, file = "data/dat_dMACS_S.Rev3.RDS")
saveRDS(dat_dMACS_S_low, file = "data/dat_dMACS_S_low.Rev3.RDS")
saveRDS(dat_dMACS_S_high, file = "data/dat_dMACS_S_high.Rev3.RDS")
